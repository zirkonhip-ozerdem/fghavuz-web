...
export async function generateMetadata({
  ...
  const [seo, corporate, t] = await Promise.all([
    getSeoPage("corporate", locale),
    getCorporate(locale),
    getTranslations({locale, namespace: "sections"}),
  ]);

  return toMetadata(corporate?.seo ?? seo, {
...
export default async function CorporatePage({
  ...
  const videoSource = corporate
    ? resolveVideoSource(corporate.videoUrl, corporate.videoMedia)
    : null;
          ...
          <ol className="mt-10 space-y-8 border-s-2 border-ink/10 ps-6 sm:mt-12 sm:space-y-10 sm:ps-8">
            {corporate?.milestones.length
              ...
              <li key={`${milestone.year}-${index}`} className="relative">
                ...
                <p className="mt-2 max-w-xl text-sm leading-6 text-ink/58">{milestone.description}</p>
              </li>
            ))
