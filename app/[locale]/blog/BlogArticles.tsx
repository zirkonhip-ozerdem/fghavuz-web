...
export function BlogArticles({articles, locale, noResultsText, detailLabel, readTimeLabel}: BlogArticlesProps) {
        ...
        >
          ...
          <div className="flex flex-1 flex-col justify-between gap-4 p-6 text-left">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-ink/70">{article.excerpt}</p>
            </div>
            <div className="flex items-center justify-between border-t border-ink/10 pt-4 text-xs uppercase tracking-[0.24em] text-ink/60">
              <span>{article.readTimeMinutes > 0 ? readTimeLabel(article.readTimeMinutes) : null}</span>
